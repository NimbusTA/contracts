...
def test_check_info(nimbus, LedgerProxy, LedgerBeacon, accounts):
    beacon = LedgerBeacon.at(nimbus.LEDGER_BEACON())
    for stash in (0x10, 0x20, 0x30):
        nimbus.addLedger(hex(stash), hex(stash + 1), 0, {'from': accounts[0]})
        ledgerProxy = LedgerProxy.at(nimbus.findLedger(hex(stash)))
...
def test_update_revision(nimbus, LedgerBeacon, LedgerMock, accounts):
    beacon = LedgerBeacon.at(nimbus.LEDGER_BEACON())
    ledgers = []

    for stash in (0x10, 0x20, 0x30):
        nimbus.addLedger(hex(stash), hex(stash + 1), 0, {'from': accounts[0]})
        ledgers.append(LedgerMock.at(nimbus.findLedger(hex(stash))))
        assert ledgers[-1].NIMBUS() == nimbus
